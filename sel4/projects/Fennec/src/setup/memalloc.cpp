...
bool configureThread() {
	...
	seL4_Error error = seL4_TCB_Configure(	globals_ptr->memory_allocator_tcb_slot, seL4_CapNull,
											...
											(seL4_Word)MEM_ALLOC_IPC_BUFFER_VADDR, globals_ptr->memory_allocator_ipc_buffer_slot);	// IPC args
...
bool allocateObjects() {
	// Create thread TCB
	seL4_Error error = seL4_Untyped_Retype(globals_ptr->bootstrap_memory_slot, seL4_TCBObject, seL4_TCBBits, seL4_CapInitThreadCNode, 0, 0, globals_ptr->memory_allocator_tcb_slot, 1);
	retFalseIfFail(error == seL4_NoError);
	...
	// Create IPC buffer
	error = seL4_Untyped_Retype(globals_ptr->bootstrap_memory_slot, seL4_X86_4K, 0, seL4_CapInitThreadCNode, 0, 0, globals_ptr->memory_allocator_ipc_buffer_slot, 1);
	retFalseIfFail(error == seL4_NoError);
	
	// Create TLS
	error = seL4_Untyped_Retype(globals_ptr->bootstrap_memory_slot, seL4_X86_4K, 0, seL4_CapInitThreadCNode, 0, 0, globals_ptr->memory_allocator_tls_slot, 1);
	retFalseIfFail(error == seL4_NoError);
...
bool mapObjects() {
	seL4_Error error = seL4_X86_Page_Map(globals_ptr->memory_allocator_ipc_buffer_slot, seL4_CapInitThreadVSpace, MEM_ALLOC_IPC_BUFFER_VADDR, seL4_ReadWrite, seL4_X86_Default_VMAttributes);
	retFalseIfFail(error == seL4_NoError);
	
	error = seL4_X86_Page_Map(globals_ptr->memory_allocator_tls_slot, seL4_CapInitThreadVSpace, MEM_ALLOC_TLS_VADDR, seL4_ReadWrite, seL4_X86_Default_VMAttributes);
	retFalseIfFail(error == seL4_NoError);
...
bool setupTLS() {
	...
	error = seL4_TCB_SetTLSBase(globals_ptr->memory_allocator_tcb_slot, tls);
	printf("Error 2: %d\n", error);
	retFalseIfFail(error == seL4_NoError);
