...
static void retypeIntoCTable() {
    seL4_CPtr untyped_cptr, return_cptr;
    seL4_Error error;
    
    assert(!outOfGraphicsCTables());
...
static void retypeWrapper(seL4_CPtr region, seL4_Word type, seL4_Word size_bits, seL4_CPtr* untyped_cptr) {
...
static void allocatePadding(seL4_CPtr untyped, seL4_Word start_addr, seL4_Word target_addr) {
    ...
    for (int i = 63; i != 12; i--) {
        if (diff & BIT(i)) {
            retypeWrapper(untyped, seL4_UntypedObject, i, nullptr);
        }
    }
}
...
static void mapFramesFromRegion(seL4_CPtr slot, seL4_Word target_paddr, seL4_Word target_vaddr, seL4_Word bytes_to_map) {
    ...
    for (seL4_Word i = 0; i < bytes_to_map/BIT(seL4_PageBits); i++) {
        ...
        retypeWrapper(slot, seL4_X86_4K, 0, &frame_cptr);

        assert(Globals::mapping_context.mapFrame(frame_cptr, target_vaddr));
        
        target_vaddr += BIT(seL4_PageBits);
    }
}

void Setup::setupGraphics() {
    // If no framebuffer data was provided, we can't set up graphics
    assert(Globals::framebuffer_info != nullptr);
    ...
    for (seL4_CPtr slot = Globals::boot_info->untyped.start; slot != Globals::boot_info->untyped.end; slot++) {
        ...
        if (start_addr <= graphics_paddr && end_addr > graphics_paddr) {
            ...
            mapFramesFromRegion(slot, graphics_paddr, graphics_vaddr, bytes_to_map);
